...
function prefix_ajax_add_foobar(){
	    ...
	    $cateoptions = "<option value='default' value='default' class='select_cate_choose' >".__('所有类别','WPWPH')."</option>";
     ...
     switch ($_GET['rtype']) {
     	case 'posts':
     		$button_value = __('插入','WPWPH').'<span>'.__(' 内容','WPWPH').'</span>';
     		break;
     	case 'urls':
     		$button_value = __('插入','WPWPH').'<span>'.__(' 链接','WPWPH').'</span>';
     		break;
     	case 'phmsg':
     		$button_value = __('同步','WPWPH');
     		break;
     }
     $args = array(
		   'public'   => true,
	...
	_e('<input type="hidden" id="hidden_post_tid" value="'.$_GET['tid'].'">
		...
		<div class="tablenav top">
         ...
         <input id="post-search-submit" class="button" type="submit" value="'.__('搜索','WPWPH').'"></input>
         </div>
    ...
    if(count($posts_array)==0){
        _e("<thead><tr><th style='text-align:center;height: 77px;'>".__('搜索结果为空....','WPWPH')."</th></tr></thead>");
    }else{
    _e("<thead><tr><th class='' width='40%'>".__('标题','WPWPH')."</th><th width='16%'><div sytle='text-align:center;'>".$typeORcate."</div></th><th width='22%'>".__('创建日期','WPWPH')."</th><th width='22%' style='text-align:center;'>".__('操作','WPWPH')."</th></tr></thead>
    	<tbody>");
        $i=1;
...
function prefix_ajax_get_insert_content(){
	if($_GET['rtype']=='posts'){
      ...
      $post_categories = wp_get_post_categories($myrow->ID);
	    $cats = '';
			...
			$cats = substr($cats,1);
		    $text_title = wp_trim_words($myrow->post_title,SYNC_TITLE_LIMIT);
		    $text_content = wp_trim_words($myrow->post_content,SYNC_CONTENT_LIMIT);
			$rpost = '#'
			         .$text_title
			         .'#'
			         .$text_content
    ...
    if(isset($_GET['imagesize']) && $_GET['imagesize']=='small'){
      ...
      $imageSize = 'sup_wechat_big';
		...
		$myrow = get_post($_GET['postid']);
	  $myrow->pic = WPWPH_HELPER_URL.'/img/'.$imageSize.'.png';
    ...
    $myrow->post_title = wp_trim_words($myrow->post_title,SYNC_TITLE_LIMIT);
	    
	  if(trim($myrow->post_excerpt)!=''){
      $myrow->post_content = wp_trim_words($myrow->post_excerpt,
                                        SYNC_EXCERPT_LIMIT);
	  }else{
	    $myrow->post_content = wp_trim_words($myrow->post_content,
                                        SYNC_EXCERPT_LIMIT);
	  }
